...
export class TeachersService {
  async list(query: ListTeachersQueryDto): Promise<{
    ...
    if (query.gender) {
      filters.push(eq(teachers.gender, query.gender))
    }
    if (query.status) {
      filters.push(eq(teachers.status, query.status))
    }
    if (query.search) {
    ...
    return {
      data: rows.map((row) => this.toTeacherView(row.teacher, row.user)),
      meta: {
        page,
        limit: query.limit,
        total,
  ...
  async getById(tenantId: string, id: string): Promise<TeacherView> {
    ...
    return this.toTeacherView(row.teacher, row.user)
  }
  ...
  async create(payload: CreateTeacherDto): Promise<TeacherView> {
    ...
    return this.toTeacherView(created, user)
  }
  ...
  async update(tenantId: string, id: string, payload: UpdateTeacherDto): Promise<TeacherView> {
    ...
    if (payload.joinDate) {
      updatePayload.joinDate = payload.joinDate
    }
    ...
    if (typeof payload.salary === "number") {
      updatePayload.salary = String(payload.salary)
    }
    if (payload.status) {
    ...
    return this.toTeacherView(updated, current.user)
  }
  ...
  private async findUserOrThrow(
    ...
    const [row] = await db
      ...
      .limit(1)
    if (!row) {
      throw new NotFoundException("Teacher user account not found in tenant")
    }
    return row
  }
  ...
  private async assertUniqueEmployeeId(
    ...
    if (ignoreTeacherId) {
      filters.push(ne(teachers.id, ignoreTeacherId))
    }
  ...
  private async assertUserNotTeacher(tenantId: string, userId: string): Promise<void> {
    const [existing] = await db
      ...
      .limit(1)
    if (existing) {
      throw new BadRequestException("User already linked to another teacher record")
    }
  }
